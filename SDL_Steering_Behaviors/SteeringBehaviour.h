...
public:
	...
	Vector2D WeightedBlending(std::vector<STEERING_TYPE> behaviours, std::vector<float> weights, Vector2D targets, Agent* agent, float radius, Vector2D targetVel, float wanderOffset, float dt);
	Vector2D PrioritizedWeightedSum(std::vector<STEERING_TYPE> behaviours, std::vector<float> weights, Vector2D targets, Agent* agent, float radius, Vector2D targetVel, float wanderOffset, float dt);
